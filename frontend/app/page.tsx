  ...
  return (
    <main className={`app-shell theme-${theme}`}>
      <aside className="sidebar">
        ...
        <nav>
          <button className={activeView === "dashboard" ? "nav-item active" : "nav-item"} onClick={() => setActiveView("dashboard")} aria-label="Dashboard"><span aria-hidden="true">📊</span> Dashboard</button>
          <button className={activeView === "projects" ? "nav-item active" : "nav-item"} onClick={() => setActiveView("projects")} aria-label="Mis expedientes"><span aria-hidden="true">📋</span> Mis expedientes</button>
        </nav>
        <div className="sidebar-footer"><div className="profile"><div className="avatar">AR</div><div><strong>Alex Rodríguez</strong><span>Administrador</span></div><button onClick={() => setAuthenticated(false)} aria-label="Opciones de perfil" title="Más opciones">⋯</button></div></div>
      </aside>
      <section className="main-area">
        <header className="topbar"><div className="breadcrumbs"><button onClick={() => setActiveView(activeView === "dashboard" ? "dashboard" : "projects")}>{activeView === "dashboard" ? "Dashboard" : "Mis expedientes"}</button>{activeView !== "dashboard" && activeView !== "projects" && selectedProject && <><b>/</b><button onClick={() => setActiveView("overview")}>{selectedProject.name}</button><b>/</b><strong>{activeView === "gantt" ? "Cronograma" : "Resumen"}</strong></>}</div><div className="top-actions"><button className="icon-button" aria-label="Buscar proyectos" title="Buscar" onClick={() => setActiveView("projects")}>🔍</button><div className="notification-wrap"><button className="icon-button notification" aria-label={`Notificaciones${notifications.length > 0 ? ` (${notifications.length})` : ''}`} aria-expanded={isNotificationsOpen} onClick={() => setIsNotificationsOpen((open) => !open)} title="Notificaciones">🔔{notifications.length > 0 && <i aria-hidden="true" />}</button>{isNotificationsOpen && <div className="notification-panel"><div className="notification-heading"><strong>Notificaciones</strong><button type="button" onClick={() => setNotifications([])}>Limpiar</button></div>{notifications.length ? notifications.map((notification) => <div className="notification-item" key={notification.id}><span className="notification-mark" /><div><strong>{notification.message}</strong><small>{notification.time}</small></div></div>) : <p className="muted">No hay actividad reciente.</p>}</div>}</div><button className="theme-toggle" onClick={toggleTheme} aria-label={`Cambiar a modo ${theme === "light" ? "oscuro" : "claro"}`} title={`Modo ${theme === "light" ? "oscuro" : "claro"}`}><span aria-hidden="true">{theme === "light" ? "🌙" : "☀️"}</span><small>{theme === "light" ? "Oscuro" : "Claro"}</small></button><div className="avatar top-avatar">AR</div></div></header>
